...
public class TableColumnAdjuster implements PropertyChangeListener, TableModelListener
{
	...
	public void adjustColumns()
	{
        ...
        rowsMaxHeights = new int[table.getModel().getRowCount()];
		for (int i = 0; i < tcm.getColumnCount(); i++)
		{
			adjustColumn(i);
		}
        for (int i=0;i<rowsMaxHeights.length;i++)
            table.setRowHeight(i, rowsMaxHeights[i]);
	}
	...
	private int getColumnDataWidth(int column)
	{
		...
		{
            Dimension cellDataSize = getCellDataSize(row, column);
            preferredWidth = Math.max(preferredWidth, cellDataSize.width);
            if (rowsMaxHeights[row] < cellDataSize.height)
                rowsMaxHeights[row] = cellDataSize.height;
	 ...
	 */
	private Dimension getCellDataSize(int row, int column)
	{
		//  Inovke the renderer for the cell to calculate the preferred width

		TableCellRenderer cellRenderer = table.getCellRenderer(row, column);
		Component c = table.prepareRenderer(cellRenderer, row, column);
        return new Dimension(c.getPreferredSize().width + table.getIntercellSpacing().width, c.getPreferredSize().height + table.getIntercellSpacing().height);
	}
...
//  Implement the PropertyChangeListener
	public void propertyChange(PropertyChangeEvent e)
	{
		...
		if ("model".equals(e.getPropertyName()))
		{
			...
			model.addTableModelListener( this );  		adjustColumns();
		}
	}
	...
	public void tableChanged(TableModelEvent e)
	{
		...
		if (e.getType() == TableModelEvent.UPDATE)
		{
			...
			if (isOnlyAdjustLarger)
			{
				...
				{
                    Dimension cellDataSize = getCellDataSize(row, column);
                    updateTableColumn(column, cellDataSize.width);
				}
			}

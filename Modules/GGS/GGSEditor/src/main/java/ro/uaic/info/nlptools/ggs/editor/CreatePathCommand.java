...
public class CreatePathCommand extends AbstractUndoableEdit {
    public GraphNode start;
    public GraphNode end;

    public void undo() throws CannotUndoException {
        GrammarEditor.theEditor.selectGraph(start.getParentGraph());
        start.getChildNodesIndexes().remove(start.getChildNodesIndexes().size() - 1);
    }

    public void redo() throws CannotRedoException {
        GrammarEditor.theEditor.selectGraph(start.getParentGraph());
        start.getChildNodesIndexes().add(end.getIndex());
    }

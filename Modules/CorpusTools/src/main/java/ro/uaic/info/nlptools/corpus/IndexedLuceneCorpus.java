...
public class IndexedLuceneCorpus implements INlpCorpus {
    ...
    public static IndexedLuceneCorpus CreateIndexFromInmemoryCorpus(File indexedCorpusFolder, InmemoryCorpus inmemoryInput) throws IOException {
        ...
        for (int i = 0; i < inmemoryInput.getSentenceCount(); i++) {
            ...
            sentDoc.add(new LongPoint("GGS:StartTokenIndex", s.getFirstTokenIndexInCorpus()));
            sentDoc.add(new LongPoint("GGS:EndTokenIndex", s.getFirstTokenIndexInCorpus() + s.getTokenCount()));

            for (SpanAnnotation spanAnnotation : inmemoryInput.getSentence(i).getSpanAnnotations()) {
                Document aDoc = new Document();
                aDoc.add(new LongPoint("GGS:StartTokenIndex", s.getFirstTokenIndexInCorpus() + spanAnnotation.getStartTokenIndex()));
                aDoc.add(new LongPoint("GGS:EndTokenIndex", s.getFirstTokenIndexInCorpus() + spanAnnotation.getEndTokenIndex()));
                ...
                sentDoc.add(new LongPoint("GGS:SpanAnnotation", annotationsIndexes.size()));
        ...
        for (int i = 0; i < inmemoryInput.getTokenCount(); i++) {
            ...
            doc.add(new LongPoint("GGS:Sentence", t.getParentSentence().getSentenceIndexInCorpus()));
            for (SpanAnnotation a : t.parentAnnotations)
                doc.add(new LongPoint("GGS:SpanAnnotation", annotationsIndexes.get(a)));
        ...
        for (int i = 0; i < tempAnnotationSearcher.getIndexReader().numDocs(); i++) {
            ...
            for (IndexableField f : doc.getFields()) {
                if (f.name().equals("GGS:StartTokenRefId"))
                    newDoc.add(new LongPoint("GGS:StartTokenIndex", tempTokenSearcher.search(new TermQuery(new Term("GGS:RefId", f.stringValue())), 1).scoreDocs[0].doc));
                else if (f.name().equals("GGS:EndTokenRefId"))
                    newDoc.add(new LongPoint("GGS:EndTokenIndex", tempTokenSearcher.search(new TermQuery(new Term("GGS:RefId", f.stringValue())), 1).scoreDocs[0].doc));
                else
                    newDoc.add(f);
            }
        ...
        for (int i = 0; i < tempTokenSearcher.getIndexReader().numDocs(); i++) {
            ...
            if (annotations != null) {
                for (int k : annotations)
                    newDoc.add(new LongPoint("GGS:SpanAnnotation", k));
            }
    ...
    public Set<String> getAllAttributeNames() {
        if (allAttributeNames == null) {
            ...
            try {
                for (LeafReaderContext context : tokenSearcher.getIndexReader().getContext().leaves()) {
                    try (LeafReader reader = context.reader()) {
                        for (FieldInfo fieldInfo : reader.getFieldInfos()) {
                            if (!fieldInfo.name.startsWith("GGS:"))
                                allAttributeNames.add(fieldInfo.name);
                        }
                    }
                }
            } catch (IOException e) {
    ...
    static class InputTextParserHandler extends DefaultHandler {
        ...
        public void endElement(String u, String l, String qName) throws SAXException {
            ...
            if (isLeaf) {//this is token
                ...
                doc.add(new LongPoint("GGS:Sentence", totalSentences));
                totalWords++;
                wordsInSentence++;
                try {
                    tokensWriter.addDocument(doc);
                ...
                doc.add(new LongPoint("GGS:EndTokenIndex", totalWords));
                doc.add(new LongPoint("GGS:StartTokenIndex", segmentStart));
                wordsInSentence = 0;
                totalSentences++;
                try {
                ...
                doc.add(new LongPoint("GGS:EndTokenIndex", totalWords - 1));
                doc.add(new LongPoint("GGS:StartTokenIndex", segmentStart));
